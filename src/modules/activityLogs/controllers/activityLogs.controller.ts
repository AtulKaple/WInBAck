...
export async function deleteLogsByRange(req, res, next) {
  try {
    ...
    if (logIds && Array.isArray(logIds) && logIds.length > 0) {
      const result = await ActivityLogModel.deleteMany({ _id: { $in: logIds } });
...
export async function getLogStats(req, res, next) {
  try {
    ...
    const [
      ...
      ActivityLogModel.countDocuments({ createdAt: { $gte: last7d } }),
      ActivityLogModel.countDocuments({ createdAt: { $gte: last30d } }),
      ...
      ActivityLogModel.countDocuments({ success: false }),
    ]);
